 ...
 */
public class MonitorMemoryInfo implements Serializable {
	...
	MonitorMemoryInfo() {
		...
		usedBufferedMemory = MonitorAccessor.getUsedBufferMemory();
		...
		usedPhysicalMemorySize = MonitorAccessor
				.getLongFromOperatingSystem("TotalPhysicalMemorySize")
				- MonitorAccessor.getLongFromOperatingSystem("FreePhysicalMemorySize");
	...
	private long buildUsedSwapSpaceSize() {
		try {
			return MonitorAccessor.getLongFromOperatingSystem("TotalSwapSpaceSize")
					- MonitorAccessor.getLongFromOperatingSystem("FreeSwapSpaceSize");
	...
	private String buildMemoryDetails() {
		...
		String osInfo = "";
		String osInfo2 = "";
		final long totalPhysicalMemorySize = MonitorAccessor
				.getLongFromOperatingSystem("TotalPhysicalMemorySize");
		if (totalPhysicalMemorySize >= 0) {
			osInfo = "Process cpu time = "
					+ integerFormat.format(
							MonitorAccessor.getLongFromOperatingSystem("ProcessCpuTime") / 1000000)
					+ " ms,\nCommitted virtual memory = "
					+ integerFormat.format(
							MonitorAccessor.getLongFromOperatingSystem("CommittedVirtualMemorySize")
									/ 1024 / 1024)
			...
			try {
				osInfo2 = ",\nFree swap space = "
						...
						+ MO + ",\nTotal swap space = "
						+ integerFormat.format(
								MonitorAccessor.getLongFromOperatingSystem("TotalSwapSpaceSize")
										/ 1024 / 1024)
						+ MO;
			} catch (final Throwable e) { // NOPMD
				// pour issues 794, dans google appengine
